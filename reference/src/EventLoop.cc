    ...
    , wakeupChannel_(new Channel(this, wakeupFd_))
{
    ...
    wakeupChannel_->setReadCallback(
        //可以替换为lambda表达式 [this]() { this->handleRead(); }
        std::bind(&EventLoop::handleRead, this)); // 设置wakeupfd的事件类型以及发生事件后的回调操作
    
    //Channel->EventLoop->Poller->EpollPoller
    wakeupChannel_->enableReading(); // 每一个EventLoop都将监听wakeupChannel_的EPOLL读事件了
}
EventLoop::~EventLoop()
{
    wakeupChannel_->disableAll(); // 给Channel移除所有感兴趣的事件：停用
    wakeupChannel_->remove();     // 把Channel从EventLoop上删除掉：注销
    ::close(wakeupFd_);
...
void EventLoop::queueInLoop(Functor cb)
{
    ...
    /** 配合doPendingFunctors使用
     * 简而言之：loop在执行回调的时候，再次调用queueInLoop加入新的回调就需要唤醒loop线程，可以更快的执行新加入的回调，而不是等到下一次poll超时
     * 详细解释：
